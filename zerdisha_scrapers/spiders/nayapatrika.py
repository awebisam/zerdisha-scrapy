"""Hybrid RSS/Scrapy spider for Naya Patrika.

This spider implements a hybrid data ingestion strategy combining RSS feed
discovery with full article content extraction. It uses Naya Patrika's
...
class NayapatrikaSpider(scrapy.Spider):
    """Hybrid RSS/Scrapy spider for Naya Patrika news articles.

    This spider uses Naya Patrika's RSS feed to discover new articles
    ...
    name: str = "nayapatrika"
    allowed_domains: List[str] = ["nayapatrikadaily.com"]
    rss_url: str = "https://www.nayapatrikadaily.com/?feed=rss2"

    def start_requests(self) -> Generator[Request, None, None]:
        """Generate initial requests by parsing the RSS feed.

        This method fetches and parses Naya Patrika's RSS feed to
    ...
    def parse_article(self, response: Response) -> Generator[ArticleItem, None, None]:
        ...
        try:
            ...
            paragraphs: List[str] = response.css('.news-content p::text').getall()

            if not paragraphs:
                self.logger.warning(
                    f"No content found using CSS selector '.news-content p' for {response.url}")
                return
            ...
            article['source_name'] = "Naya Patrika"
    ...
    def _extract_publication_date(self, response: Response) -> Optional[str]:
        ...
        try:
            ...
            if time_element:
                try:
                    # Parse the datetime attribute and convert to ISO format
                    parsed_date = datetime.fromisoformat(time_element.replace('Z', '+00:00'))
                    return parsed_date.date().isoformat()
                except ValueError:
                    self.logger.debug(
                        f"Could not parse datetime attribute: {time_element}")

            # Try to extract from meta property
            ...
            if published_meta:
                try:
                    ...
                    self.logger.debug(
                        f"Could not parse published_time meta: {published_meta}")

            # Fallback: Extract date from URL structure
            # URL format: https://www.nayapatrikadaily.com/YYYY/MM/DD/article-slug
